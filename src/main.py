...
users_processed_total = Counter('users_processed_total', 'Total users processed', ['source'])
...
def get_users():
    ...
    try:
        ...
        return jsonify({'users': ['user1', 'user2'], 'timestamp': time.time()}), 200
        ...
        http_request_duration_seconds.labels('/api/users').observe(duration)
        active_requests.dec()

# === Эндпоинт для симуляции ошибок (для тестирования алертов) ===
@app.route('/api/fail', methods=['POST'])
def simulate_failure():
    http_requests_total.labels('POST', '/api/fail', '500').inc()
    return jsonify({'error': 'Simulated failure'}), 500

if __name__ == '__main__':
    # Важно: port должен совпадать с тем, что указан в docker-compose
    app.run(host='0.0.0.0', port=8080)  # или 5000, как у вас сейчас
